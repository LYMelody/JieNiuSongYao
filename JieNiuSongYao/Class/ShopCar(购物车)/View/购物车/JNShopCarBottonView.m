...
@property(nonatomic,strong)UIButton *delecteButon;
...
@implementation JNShopCarBottonView
...
- (void)setUpViews {
    ...
    [self addSubview:self.delecteButon];
...
- (void)changeShopCarBottomViewWithStatus:(BOOL)status {
    ...
    self.delecteButon.hidden = !status;
    
    
    
}

//给JNShopCarBottomView设置值
- (void)configureShopCarBottomViewWithTotalPrice:(float)totalPrice totalCount:(NSInteger)totalCount isAllselected:(BOOL)isAllSelected {
    ...
    self.starButton.enabled = totalPrice && totalCount;
    self.delecteButon.enabled = totalCount && totalPrice;
    if (self.settleButton.isEnabled) {
        ...
        [self.delecteButon setBackgroundColor:[UIColor colorWithRed:0.918 green:0.141 blue:0.137 alpha:1]];
        ...
        [self.delecteButon setBackgroundColor:[UIColor lightGrayColor]];
...
- (void)renderWithTotalPrice:(NSString *)totalPrice {
    ...
    [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:210/255.0 green:50/255.0 blue:50/255.0 alpha:1] range:[self.totalPriceLable.text rangeOfString:totalPrice]];
...
- (void)allSelectButtonAction {
    ...
    if (self.shopCarBottomViewAllSelectBlock) {
        self.shopCarBottomViewAllSelectBlock(self.allSelectButton.selected);
    }
...
- (void)settleButtonAction {
    ...
    if (self.shopCarBottomViewSettleBlock) {
        self.shopCarBottomViewSettleBlock();
    }
    
}


- (void)delecteButonAction {
    
    if (self.shopCarBottomViewDelectBlock) {
        self.shopCarBottomViewDelectBlock();
    }
...
- (void)starButtonAction {
    ...
    if(self.shopCarBottomViewStarBlock ) {
        
        self.shopCarBottomViewStarBlock();
...
- (UIButton *)allSelectButton {
    ...
    if (_allSelectButton == nil) {
        ...
        [_allSelectButton setTitleColor:[UIColor colorWithRed:150/255.0 green:150/250.0 blue:150/255.0 alpha:1] forState:UIControlStateNormal];
...
- (UIButton *)delecteButon {
    
    if (_delecteButon == nil) {
        _delecteButon = [UIButton buttonWithType:UIButtonTypeCustom];
        [_delecteButon setTitle:@"删除" forState:UIControlStateNormal];
        [_delecteButon setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_delecteButon setBackgroundColor:[UIColor lightGrayColor]];
        _delecteButon.titleLabel.font = [UIFont systemFontOfSize:13];
        [_delecteButon addTarget:self action:@selector(delecteButonAction) forControlEvents:UIControlEventTouchUpInside];
        _delecteButon.enabled = NO;
        _delecteButon.hidden = YES;
    }
    return _delecteButon;
}
...
- (UIButton *)starButton {
    ...
    if (_starButton == nil) {
        ...
        [_starButton setBackgroundColor:[UIColor lightGrayColor]];
...
- (UIView *)separateLine {
    if (_separateLine == nil) {
        ...
        _separateLine.backgroundColor = [UIColor colorWithRed:233/255.0 green:233/25.0 blue:233/255.0 alpha:1];
    }
...
- (void)layoutSubviews {
    ...
    [self.starButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.delecteButon.mas_left);
        make.width.equalTo(@100);
    }];
    
    [self.delecteButon mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.right.bottom.equalTo(self);
        make.width.equalTo(@100);
    }];
