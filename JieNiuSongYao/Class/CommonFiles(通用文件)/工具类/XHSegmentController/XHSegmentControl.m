...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        self.lineLayer.frame = CGRectMake(CGRectGetMinX(item.frame) + ((CGRectGetWidth(item.frame) - [XHSegmentItem caculateWidthWithtitle:item.title titleFont:item.titleFont])/2 + 20), CGRectGetHeight(item.frame) - self.lineWidth, [XHSegmentItem caculateWidthWithtitle:item.title titleFont:item.titleFont] - 46, self.lineWidth);
